...
// variable to hold time of CPU
uint64_t timerTick = 0;

// holds the variable of times Disable All interrput (used for nesting)
uint8_t DisableAllInterrupts_count = 0;
// holds the variable of times suspending all interrput (used for nesting)
uint8_t SuspendAllInterrupts_count = 0;
// holds the variable of times suspending OS interrput (used for nesting)
uint8_t SuspendOSInterrupts_count = 0;
// Holds the variable to determine next queue to run. Each bit corresponds to a Priority which activates correspond Queue.
uint32_t osek_readySet; /* bitmask of threads that are ready to run */

// an array that holds all queues currently used in order from low to high priority
...
// holds the value of current queue which is running
OsekQueue_t *volatile osek_currQueue;

// holds the value of next queue to be switched in next context wtich interrupt
OsekQueue_t *volatile osek_nextQueue;

/*
Parameter (In): oq OsekQueue_t type pointer , stkSize the stack size of oq
Parameter (Out):  none
Description: push a task to back of tasks inside queue
*/
...
/*
Parameter (In): oq OsekQueue_t type pointer
Parameter (Out):  none
Description: returns the top of tasks inside queue oq and delete the task from queue
*/
...
Parameter (In): oq OsekQueue_t type pointer
Parameter (Out):  none
Description: returns the top of tasks inside queue oq without changing queue
*/
...
Parameter (In): none
Parameter (Out):  none
Description: The main dispatcher for choosing which Queue to execute its task
*/
...
Parameter (In): oq OsekQueue_t type pointer , stkSize the stack size of oq
Parameter (Out):  none
Description: a routine to fill stack of a queue
*/
...
Parameter (In): none
Parameter (Out):  none
Description: Initialize Queues stak pointer for first context switch , inject dispatcher function to
PC register inside stack of queues and flll all unused stack with DEADBEAF pattern
*/
void initQueue(void)
{
...
Parameter (In): none
Parameter (Out):  none
Description: Initialize operating system variables such as Queues, osek_readySet global variable, task state and first
idle task activation and configuring interrupts.
*/
void InitOS()
{
	initQueue();
